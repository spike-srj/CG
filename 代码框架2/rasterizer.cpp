        ...
        if (SSAA){
            for (int x=0;x<width;x++){
                for (int y=0;y<height;y++)
                {
                    ...
                    color /=4;
                    set_pixel(Eigen::Vector3f(x,y,1.0f),color);  //frame_buf
                }
            }
        }
    }
                ...
                for (float i=0.25; i<1; i+=0.5)
                {
                    ...
                    for (float j=0.25; j<1; j+=0.5)
                    {
                        ...
                        {   
                            ...
                            auto tup = computeBarycentric2D(x+i, y+j, t.v);
                            ...
                            if (z_interpolated < depth_buf_2xSSAA[index][index2])//
                            {
                                Eigen::Vector3f p;
                                p << i,j,z_interpolated;
                                frame_buf_2xSSAA[get_index(x,y)][index2] = t.getColor();
                                depth_buf_2xSSAA[get_index(x,y)][index2] = z_interpolated;
                                inside_count++;
